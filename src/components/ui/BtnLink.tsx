...
const BtnLink: React.FC<Props> = forwardRef<HTMLAnchorElement, Props>(
    ({ href, btnType = EnumBtn.default, className = '', ...props }, ref) => (
        <Link
            ...
            href={href}
            className={cn(
                `relative flex items-center justify-center gap-1 w-full sm:w-fit sm:min-w-32 h-10 lg:h-11 px-4 rounded text-white transition-opacity duration-300 hover:opacity-80 ${className}`,
                {
